...
(function (global, factory) {
    ...
    console.dir(`定义了computed的合并策略`);
    /**
     * @说明 选项 props、methods、inject、computed 的合并策略是完全一致的.
     * @注意 父选项始终是可用的,但是子选项会优先级更高,会覆盖父选项.
     * @参数 { parentVal: 父选项, childVal: 子选项, vm: Vue实例, key: 类型(props/methods/inject/computed) }
     */
    strats.props =
    strats.methods =
    strats.inject =
    strats.computed = function (parentVal, childVal, vm, key) {
        console.log('调用 strats.computed 合并策略')
        if (childVal && "development" !== "production") {
            // 如果存在childVal参数,并且不为生产环境,需要判断childVal是否为对象;不符合条件会警告
            assertObjectType(key, childVal, vm);
        }
        if (!parentVal) return childVal; // 当没有父选项的时候,返回子选项的值
        var ret = Object.create(null); // 创建一个空对象 -> ret
        extend(ret, parentVal); // 将父选项的值拷贝到ret中
        if (childVal) {
            extend(ret, childVal); // 如果子选项的值存在,将子选项的值拷贝到ret中
            // 这里可以推断子选项的优先级更高
        }
        return ret;
    };
     ...
     *
     * @作用 将两个配置对象(选项对象)合并成一个新的对象
     * @说明 这是实例化和继承的核心逻辑(核心用法)
     * @parent resolveConstructorOptions函数的返回值, 是Vue构造函数的options
     * @child 当前配置项/子配置项, 是我们创建new Vue时自定义的options
     * @vm 实例 this 对象本身
     */
    function mergeOptions(parent, child, vm) {
        {checkComponents(child);} // 校验是否为Component || 校验组件名的合法性

        if (typeof child === "function") {
            // 如果传入的child是函数, 就取他的option
            child = child.options;
        }

        normalizeProps(child, vm); // 规范化Props属性
        normalizeInject(child, vm); // 规范化Inject属性
        normalizeDirectives(child); // 规范化Directives属性

        /**
         * 如果是非合并的选项(为经过处理/原始选项对象/并非mergeOptions调用结果),
         * 判断, 如果child存在extends/mixin属性, 就将child的extends/mixin属性添加到parent中;
         * 这是对extends/mixin的合并处理
         */
        if (!child._base) {
            if (child.extends) {
        ...
        var options = {};
        var key;
        // 针对不同的属性(key), 有不同的合并策略
        // console.log("%cmergeOptions", "color: blue;")
        for (key in parent) {
            mergeField(key);
        }
        for (key in child) {
        ...
        // console.log('合并配置项之后的返回值 >>> ', options);
        return options;
        // 经过mergeOptions合并后, 增加了_base属性; 可用于区分
    }
    ...
    // console.log(1, typeof Promise !== "undefined" && isNative(Promise))
    // console.log(2, !isIE &&
    ...
    // console.log(3, typeof setImmediate !== "undefined" && isNative(setImmediate))
...

    {
        ...
        var hasHandler = {
            ...
            has: function has(target, key) {
                ...
                // * 拦截"key in target", 返回boolean值; 首字符为'_'
                var isAllowed =
                    allowedGlobals(key) ||
                    (typeof key === "string" &&
                        key.charAt(0) === "_" &&
                        !(key in target.$data));
                // 访问一个没有定义在实例(原型)上的属性时,会报警告
                if (!has && !isAllowed) {
        ...
        /**
         * 1. 判断是否支持Proxy
         * 2. 如果支持Proxy就读取options, 然后为实例添加 _renderProxy 属性
         * 3. 在读取 _withStripped 时进行了拦截:
         *    - 当属性存在render并且render中有_withStripped时执行getHandler拦截
         *    - 其余情况使用hasHandler拦截
         */
        initProxy = function initProxy(vm) {
            // 需要确定使用哪个代理程序
            if (hasProxy) {
                var options = vm.$options; // 使用合并之后的options
                var handlers =
                    // ! _withStripped 只在测试代码出现, 一般都是false, 这里常赋值: hasHandler
    ...
    var Watcher = function Watcher(vm, expOrFn, cb, options, isRenderWatcher) {
        ...
        // getter的解析表达式
        // TODO: user componend render 的watcher都会走到这里面
        if (typeof expOrFn === "function") {
            this.getter = expOrFn;
        } else {
            this.getter = parsePath(expOrFn); // 得到getter
            // console.log('getter: ', expOrFn, this.getter)
            if (!this.getter) {
                this.getter = noop;
                warn(
    ...
    function initState(vm) {
        ...
        var opts = vm.$options;
        /**
         * ! 在initState中判断是否具有Porps/methods/data/computed/watch属性, 分别执行对应的初始化方法
         */
        if (opts.props) {
        ...
        // console.log("在initState方法中判断 >>> if(opts.computed) initComputed >>> ", opts.computed);
        if (opts.computed) {
            initComputed(vm, opts.computed);
        }
        console.log("在initState方法中判断 >>> if(opts.watch) initWatch >>> ", opts.watch);
    ...
    function initComputed(vm, computed) {
        console.log("%cinitComputed >>> 初始化computed属性", "color:red; font-weight: bold;");
        // $flow-disable-line
        // ! 定义一个用于存放computed的watcher的空对象 <_computedWatchers>, 挂载到vm上
        var watchers = (vm._computedWatchers = Object.create(null));

        // computed properties are just getters during SSR
        // ? 计算属性在SSR中只是getters, 应该是SSR仅做接收, 存在get不存在set
        ...
        for (var key in computed) {
            ...
            // console.log('计算属性: ', key, ' >>> ', getter)
            if (getter == null) {
                warn('计算属性"' + key + '"缺少getters, 请检查.', vm);
            }

            if (!isSSR) {
                // create internal watcher for the computed property.
                // ! 为计算属性创建内部watcher程序 (创建观察者)
                // ! 这是 computedWatcher, 不同于 renderWatcher
                watchers[key] = new Watcher(
                    vm,
                    getter || noop,
                    noop, // 发生改变时会执行的回调函数, 这里传入的是空值
                    computedWatcherOptions // 这里传入的参数 { lazy: true } 这也是computed惰性监听的原因
                );
            }
            console.log(`%c${key} >>> watcher: `, 'color: red;', watchers[key])

            // component-defined computed properties are already defined on the
            // component prototype. We only need to define computed properties defined
            // at instantiation here.
            /**
             * ! 组件定义的计算属性已经挂载到了组件原型上, 所以这里只需要定义在实例化时定义的计算属性
             */
            if (!(key in vm)) {
                defineComputed(vm, key, userDef);
            } else {
                // ! computed中的属性, 不能和data/props/methods中的属性重名
                if (key in vm.$data) {
    ...
    /**
     * @作用 将computed的每个属性都挂载到实例vm上; getter/setter
     * @功能 惰性监听 -> 在初始化的时候并取值计算, 在请求时根据dirty计算
     */
    function defineComputed(target, key, userDef) {
        var shouldCache = !isServerRendering(); // 是否为SSR
        if (typeof userDef === "function") { // 如果是函数, 只有getter
            sharedPropertyDefinition.get = shouldCache
                ? createComputedGetter(key)
                : createGetterInvoker(userDef);
            sharedPropertyDefinition.set = noop;
        } else { // 如果是对象, 则有getter/setter
        ...
        if (sharedPropertyDefinition.set === noop) {
            // computed properties without setter are read-only.
    ...
    function createComputedGetter(key) {
        return function computedGetter() {
            ...
            if (watcher) {
                // ! dirty属性是缓存的关键, 取自watcher中的dirty属性
                if (watcher.dirty) {
                    watcher.evaluate();
                }
                if (Dep.target) {
    ...
    function stateMixin(Vue) {
        ...
        /**
         * @Watcher 对象生成
         * @immediate 实际上就是立即调用对应的回调函数
         * @unwatch 逻辑处理即取消watch监听
         */
        Vue.prototype.$watch = function (expOrFn, cb, options) {
            var vm = this;
            if (isPlainObject(cb)) {
                // ! 如果cb是一个对象, 创建一个Watcher并传入参数
                return createWatcher(vm, expOrFn, cb, options);
            }
            options = options || {};
            /**
             * 在这里实例化一个 user Watcher
             * user/lazy/sync属性都会影响watcher的触发逻辑
             */
            options.user = true;
            /**
             * @Watcher构造函数的四个参数
             * @vm 对应的vue实例
             * @expOrFn 用于生成getter表达式，watcher对象的核心执行逻辑;;  就是watch中的每一个key
             * @cb 对应的回调函数
             * @options 配置项
             */
            var watcher = new Watcher(vm, expOrFn, cb, options);
            // 注册成功之后, 一旦数据发生变化, 就会调用watcher的run方法,执行cb
            if (options.immediate) {
                // 如果immediate是true, 立即执行一次
    ...
    function initMixin(Vue) {
        ...
        */
        Vue.prototype._init = function (options) {
            // console.log('Vue: ', JSON.stringify(options))
            // 此时的options仅仅是包含el/data/computed等属性, 还没有添加$attr/observer等属性
            var vm = this;
            // 给Vue实例挂一个属性: _uid，用来记录实例的id
            vm._uid = uid$3++;
            // 标记开始结束节点,用于Vue插件的截取
            var startTag, endTag;
            /* istanbul ignore if */
            /**
             * @功能 用于支持开发模式下的调试 | 猜测是Chrome中的Vue相关的插件
            */
            if (config.performance && mark) {
            ...
            /**
             * @_isVue 避免被observer观测到
             * @说明 因为vm上面挂载的变量都会被检测,这是Vue的特性,依赖收集响应式
            */
            vm._isVue = true;
             ...
             * @合并选项
             * 两种合并策略:
             * 1.Component组件,在vnode创建组件实例时,执行initInternalComponent进行组件内部配置合并
             * 2.非组件形式,根实例创建时,执行mergeOptions进行配置合并
             * @结果 合并配置项,将属性合并到vm上,并且添加$options属性
             */
            if (options && options._isComponent) {
                /**
                 * 优化内部组件实例化的逻辑,因为动态选项的合并很慢,并且内部组件不需要特殊处理
                ...
                console.log('调用 mergeOptions 方法')
                vm.$options = mergeOptions(
                    resolveConstructorOptions(vm.constructor), // 解析构造函数的选项
                    options || {},
                    vm
                );
            }
            console.log(`%c完成mergeOptions, 开始初始化Computed`, 'color:green')
            // console.log('合并完毕配置项: ', JSON.stringify(vm.$options))

            /* istanbul ignore else */
            // ! 初始化代理(Proxy), 代理vm上的属性; 给 "渲染函数的作用域" 添加代理
            { initProxy(vm); }

            /**
             * expose real self
             * 定义私有变量属性, 将自己保存到自身属性上, 用于访问实例的数据和方法
             */
            vm._self = vm;
    ...
    /**
     * @参数vm 组件实例vm == this
     * @参数options 构造函数中传入的options
     * @触发条件 存在options并且 _isComponent == true
    */
    function initInternalComponent(vm, options) {
        // doing this because it's faster than dynamic enumeration.
        // 以下挂载方法, 比动态枚举更快

        // 首先利用object.create方法, 将构造函数的options挂载到vm.$options的__proto__上
        var opts = (vm.$options = Object.create(vm.constructor.options));

        // 然后将options中的_parentVnode和parent挂载到vm.$options上
        var parentVnode = options._parentVnode;
        opts.parent = options.parent; // parent是组件的根实例 #app
        opts._parentVnode = parentVnode; // parentVnode是绑定生成的一个Vnode对象
        // console.log('parent: ', opts.parent)
        // console.log('_parentVnode: ', opts._parentVnode)
     ...
     * @说明 Vue实例初始化阶段
     * @功能 Vue实例化 / 混合继承? / 实例继承?
     * @其他 如果未使用new关键字创建实例,会弹出一个警告
     */
    function Vue(options) {
        console.log("Vue实例初始化阶段 >>> 调用init方法合并options")
    ...
    console.log("%cStart >>> init/state/event/lifecycle/render", "color: red; font-weight: bold");
    initMixin(Vue); // 初始化
    stateMixin(Vue); // 状态
    eventsMixin(Vue); // 事件
    ...
    function initExtend(Vue) {
        ...
        Vue.extend = function (extendOptions) {
            ...
            console.log('在extend阶段也会合并options')
